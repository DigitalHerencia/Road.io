...
function decryptFields(integrations: IntegrationSettings, fields: string[]) {
  const result = { ...integrations } as Record<string, unknown>;
  for (const key of fields) {
    if (typeof result[key] === 'string' && result[key]) {
      try {
        result[key] = decryptString(result[key] as string);
      } catch {
