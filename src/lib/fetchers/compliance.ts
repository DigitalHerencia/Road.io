...
export async function listComplianceAuditLogs(orgId: number, limit = 10): Promise<AuditLog[]> {
  ...
  return res.rows.map(row => ({
    ...row,
    orgId,
    userId: 0, // Default value since not available in query
    ipAddress: null,
    userAgent: null,
  }))
}
...
export async function getComplianceConfig(orgId: number): Promise<ComplianceConfig> {
  ...
  const cfg = (org?.settings as Record<string, unknown>)?.complianceConfig as ComplianceConfig | undefined
