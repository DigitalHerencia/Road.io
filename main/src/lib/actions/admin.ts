...
const tenantConfigSchema = z.object({
  ...
  resourceLimit: z.coerce.number().min(0).optional(),
  notes: z.string().max(500).optional(),
});

export async function updateTenantConfigAction(data: FormData) {
  const admin = await requireRole(SystemRoles.ADMIN);
  const parsed = tenantConfigSchema.parse({
    dataRetentionDays: data.get("dataRetentionDays"),
  ...
  await createAuditLog({
    ...
    details: { updatedBy: admin.id, tenantConfig: parsed },
  });

  revalidatePath("/dashboard/admin/tenant");
  return { success: true };
}

export async function verifyTenantIsolationAction() {
  const admin = await requireRole(SystemRoles.ADMIN);
